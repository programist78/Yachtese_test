...
const layout: FC = () => {
    ...
    const getLink = async (productId:string, priceKey:string) => {
        return buy({
            variables: {
    ...
    return <main className={c.main}>
        <section>
            <Title className={c.title}>Yacht Subscription Options</Title>
        ...
        <section className={classNames('container', c.packages)}>
            <article className={classNames('block', c.package)}>
                ...
                <span className={c.price}>25$</span>
                <button className={c.buy} onClick={() => {
                    getLink('prod_OlZ0IMmmQ8qwfq', 'price_1Ny1sGJjsA7WDmLer0ZD5CCW').then(({ data, errors }) => {
